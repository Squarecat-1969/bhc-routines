...
npm run late-edition -- --dry-run --limit 3   # ALL EIGHT PASSES chained, one shared Run_ID, never run against prod yet
npm run late-edition -- --live                # the real nightly run
npm run inspect:activity-log   # read-only — dumps real Activity_Log header/sample rows

# Contacts Triage — a separate routine from Late Edition, NOT one of its passes.
npm run contacts-triage:dry    # enumerate + exclude + score + report, writes nothing
npm run contacts-triage -- --dry-run --no-llm    # free run: deterministic distribution only
npm run contacts-triage -- --dry-run --llm       # same, but spends the STEP 4 calls
npm run contacts-triage -- --live --llm          # writes Contacts_Triage_Queue + Contact_Exclusions
```

Migration status: **The full Late Edition rebuild is complete, fully live-verified end to end, running on a real schedule, and the old agentic routine is decommissioned, 2026-07-19.** All eight passes (0, 1, 2, 2.5, 3, 4, 4.5, 5) have real, tested TypeScript implementations, and the combined orchestrator (`src/passes/orchestrator/`, `npm run late-edition`) chains all eight in one process with one shared `RUN_ID`. **A full `--live` run at complete real production scale (2855 Contacts, 2450 Master_ID, 44 Attio pipeline entries, 2216 PASS 4.5 targets, all 79 PASS 2.5 clusters) completed cleanly in ~8m49s — zero aborts, zero warnings.** Every pass has been individually live-verified with real writes confirmed correct; real bugs were found and fixed throughout the build (token limits, a missing Anthropic-response diagnostic, two independent instances of a numeric-date-serial bug, two PASS 3 drift-notes bugs, a warnings-visibility gap in the combined report) — see `docs/pass2-notes.md`, `docs/pass2_5-notes.md`, `docs/pass3-notes.md`, `docs/pass5-notes.md`, `docs/late-edition-orchestrator-notes.md` for the full writeups. **The GitHub Actions workflow (`.github/workflows/late-edition.yml`) now runs automatically 11pm Sun-Thu Pacific, live** (a real double-run-per-night bug in the DST-covering cron pair was caught and fixed with a runtime guard before this went live). **Bobby has deleted the old agentic Late Edition routine's cron trigger** — the two systems no longer run in parallel; the new orchestrator is the sole nightly execution path. `routines/BHC_Late_Edition.md` (the old routine's prompt-spec) is marked superseded with a notice at the top but kept as historical reference — every pass's implementation was built by transcribing it, and every `docs/*-notes.md` file cites it by section name when explaining a resolved ambiguity or deliberate deviation. **The migration is complete.** What remains is unrelated to Late Edition itself: Aida UX gaps, the `bhc-aida` Accept-handler follow-up for `placeholder_reconciliation` rows. The Name_Conflicts backlog (31 conflicts across both Reconciler and Late Edition sources) was fully cleared 2026-07-19, and the diacritic-stripping pattern behind most of it was root-caused the same day — see the CHANGELOG entry "Name_Conflicts now classifies DIACRITIC_ONLY vs STRUCTURAL" and `docs/name-conflicts-panel-spec.md` for the fix, the finding, and the Aida-side spec it unblocks.

**Contacts Triage (`src/passes/contacts-triage/`, added 2026-08-08) is a new routine, not a Late Edition pass.** It scores Attio person records that have no `bhc_contact_id` and stages a review queue for Aida's Contacts triage page; it never mints, never archives, never writes to Attio, and writes only `Contacts_Triage_Queue` and `Contact_Exclusions`. **Dry runs are live-verified against production Attio.** (An earlier note here said the first live write failed and the queue was still empty — stale as of 2026-09-01: a live read returned 97 `Contacts_Triage_Queue` rows and 240 `Contact_Exclusions` rows.) **STEP 1b suppression shipped 2026-09-01 (`49cb8ef`)** — it reads `Master_ID` SUPERSEDED rows and `Contact_Exclusions` before scoring, and suppresses 231 of 251 unbridged records on live data. **That 231 is not the new capability's contribution:** the two signals are disjoint, 229 come from `Contact_Exclusions` matching that already existed, and **2 from the new SUPERSEDED half — both Raymond Yang**. See `docs/contacts-triage-notes.md` #19 (numbered 19 because `#15`–`#18` are already cited from code for notes never written into that file). Its GitHub Actions workflow is deliberately dispatch-only and has no schedule. Scoring runs on Attio's computed connection signals, not on message metadata: the Attio Emails scope does not exist for workspace API tokens, which is a product boundary and not a misconfiguration (#15). STEP 4 is gated on readable evidence rather than band position (#20). Weights and thresholds remain an explicit starting hypothesis. Both tabs must exist before a live run — the Sheets proxy cannot create tabs.
