...
**Contacts Triage (`src/passes/contacts-triage/`, added 2026-08-08) is a new routine, not a Late Edition pass.** It scores Attio person records that have no `bhc_contact_id` and stages a review queue for Aida's Contacts triage page; it never mints, never archives, never writes to Attio, and writes only `Contacts_Triage_Queue` and `Contact_Exclusions`. **Dry runs are live-verified against production Attio.** (An earlier note here said the first live write failed and the queue was still empty — stale as of 2026-09-02: a live read returned 97 `Contacts_Triage_Queue` rows and 240 `Contact_Exclusions` rows.) **STEP 1b suppression shipped 2026-09-02** — it reads `Master_ID` SUPERSEDED rows and `Contact_Exclusions` before scoring, and on live data suppresses 231 of 251 unbridged records; see `docs/contacts-triage-notes.md` #15. Its GitHub Actions workflow is deliberately dispatch-only and has no schedule. Scoring runs on Attio's computed connection signals, not on message metadata: the Attio Emails scope does not exist for workspace API tokens, which is a product boundary and not a misconfiguration (#15). STEP 4 is gated on readable evidence rather than band position (#20). Weights and thresholds remain an explicit starting hypothesis. Both tabs must exist before a live run — the Sheets proxy cannot create tabs.
