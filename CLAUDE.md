...
Migration status: **The full Late Edition rebuild is complete, 2026-07-19 — all eight passes (0, 1, 2, 2.5, 3, 4, 4.5, 5) now have real, tested TypeScript implementations, AND a combined orchestrator now chains all eight in one process with one shared `RUN_ID`.** PASS 4, PASS 4.5, PASS 1, and PASS 0 are live-run confirmed against production. PASS 2 is fully built and live-verified — 31 real threads processed across dry/live runs, two real bugs found-fixed-reverified, one prompt-quality fix reverified clean on a harder batch. PASS 2.5 and PASS 5 are both live-verified with real writes confirmed correct (real bugs found and fixed in each — see their notes docs). PASS 3 is verified correct via `--dry-run` but has never actually posted to `#aida`. **The combined orchestrator (`docs/late-edition-orchestrator-notes.md`) is built and tested (400/400) but has never been run end-to-end against production** — it closes PASS 3's previously-documented drift-alert gap by passing PASS 2's drift warnings directly into PASS 3 in memory, deliberately does NOT restructure any individual pass's own Sheets/Attio reads (too risky for marginal gain on already-live-verified code), and surfaced one genuinely valuable confirmation while being built: the dry-run safety guarantee holds even under a full 8-pass chained run, not just each pass in isolation (a dry-run PASS 2 write is correctly invisible to a same-run PASS 3 read — traced down as a real finding, not a bug). See `docs/pass2-notes.md`, `docs/pass2_5-notes.md`, `docs/pass3-notes.md`, `docs/pass5-notes.md`, `docs/late-edition-orchestrator-notes.md` for each piece's real findings and resolved ambiguities. **What's left before this replaces the old agentic routine**: the orchestrator's own first live run (start with `--dry-run --limit N`, same discipline as every pass's own first live run), PASS 3's first real post to `#aida`, and the still-open items from before (Name_Conflicts backlog, Aida UX gaps, the `bhc-aida` Accept-handler follow-up, the diacritic bug, the old-vs-new routine decommissioning decision).
